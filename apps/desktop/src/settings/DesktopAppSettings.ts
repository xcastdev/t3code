...
export interface DesktopSettings {
  ...
  /** The desktop either owns a local primary process or securely attaches to a loopback server. */
  // Runtime normalization always provides this; optionality preserves source
  // compatibility for test/factory fixtures compiled against the former shape.
...
const DesktopSettingsDocument = Schema.Struct({
  ...
  // Decode this separately so malformed persisted credentials never silently
  // turn into a managed backend and unexpectedly acquire a process.
...
export function normalizePrimaryBackendPreference(
  ...
  if (candidate.mode === "managed") return { mode: "managed" };
  ...
  if (
    !fields.every((field) => typeof candidate[field] === "string" && candidate[field].length > 0) ||
  ...
  const expiresAt = Date.parse(candidate.bearerExpiresAt as string);
  if (endpoints === null || !Number.isFinite(expiresAt) || expiresAt <= now) {
    return invalidPrimaryBackend("Stored attached backend settings are invalid.");
  }
  return {
    mode: "attached",
...
function toDesktopSettingsDocument(
  ...
  if (settings.primaryBackend?.mode !== "managed" && settings.primaryBackend !== undefined) {
...
function setPrimaryBackendPreference(
  ...
  if (settings.primaryBackend?.mode === preference.mode) {
    if (preference.mode === "managed") return settings;
    if (
