...
describe("primary backend IPC", () => {
  it.effect("returns redacted state and relaunches after a selection change", () =>
    Effect.gen(function* () {
      ...
      const lifecycle = DesktopLifecycle.DesktopLifecycle.of({
      ...
      yield* useManagedPrimaryBackend
        .handler(undefined)
        .pipe(
          Effect.provideService(DesktopAttachedBackend.DesktopAttachedBackend, attachedBackend),
          Effect.provideService(DesktopLifecycle.DesktopLifecycle, lifecycle),
  ...
  it.effect("preserves a typed credential-refresh authorization failure", () =>
    Effect.gen(function* () {
