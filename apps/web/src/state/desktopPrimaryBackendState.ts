...
class DesktopPrimaryBackendStateUnavailableError extends Schema.TaggedError<DesktopPrimaryBackendStateUnavailableError>()(
...
class DesktopPrimaryBackendStateLoadError extends Schema.TaggedError<DesktopPrimaryBackendStateLoadError>()(
  "DesktopPrimaryBackendStateLoadError",
  { cause: Schema.Defect() },
) {
  override get message(): string {
    return "Failed to load desktop primary backend state.";
  }
}
...
export function createDesktopPrimaryBackendStateAtom(
  ...
  const loadState = Effect.fn("loadDesktopPrimaryBackendState")(function* () {
    ...
    if (!bridge) return yield* new DesktopPrimaryBackendStateUnavailableError();
