...
import {
  desktopPrimaryBackendStateAtom,
  refreshDesktopPrimaryBackendState,
} from "~/state/desktopPrimaryBackendState";
...
export function describeDesktopPrimaryBackend(state: DesktopPrimaryBackendState | null): string {
  if (state?.mode === "attached") return `Using the T3 server at ${state.httpBaseUrl}.`;
  if (state?.mode === "invalid-attached") {
    return "The saved attached backend preference needs recovery before T3 Code can connect.";
  }
  if (state?.mode === "managed") return "Started and managed by the desktop app.";
