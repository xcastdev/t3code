...
const expectSingleEvent = <Type extends OrchestrationEvent["type"]>(
...
it.layer(NodeServices.layer)("MCP catalog decider invariants", (it) => {
  ...
  it.effect("preserves an established catalog link across generic session updates", () =>
    Effect.gen(function* () {
      const threadId = ThreadId.make("thread-with-catalog");
      ...
      const sessionSet = yield* decide(
        {
          type: "thread.session.set",
          commandId: CommandId.make("generic-session-update"),
          threadId,
          session: {
            ...existingSession,
            status: "running",
            mcpCatalogSessionId: undefined,
          },
      ...
      expect(sessionSet).toMatchObject({
        type: "thread.session-set",
        payload: { session: { mcpCatalogSessionId: catalogSessionId } },
      });
      const explicitReplacement = yield* decide(
        {
          type: "thread.session.set",
          commandId: CommandId.make("explicit-catalog-replacement"),
          threadId,
          session: {
            ...existingSession,
            mcpCatalogSessionId: McpCatalogSessionId.make("catalog-session-replacement"),
          },
          createdAt: "2026-01-01T00:00:01.000Z",
        },
        readModel,
      );
      expect(explicitReplacement).toMatchObject({
        type: "thread.session-set",
        payload: {
          session: { mcpCatalogSessionId: McpCatalogSessionId.make("catalog-session-replacement") },
        },
      });
      const afterSessionSet = yield* projectEvent(readModel, {
        ...expectSingleEvent(sessionSet, "thread.session-set"),
        sequence: 1,
      });

      const applied = yield* decide(
        {
          type: "thread.mcp-catalog.applied",
          commandId: CommandId.make("catalog-applied"),
          threadId,
          mcpCatalogSessionId: catalogSessionId,
          revision: 0,
          appliedCatalog: [],
      ...
      expect(applied).toMatchObject({
        type: "thread.mcp-catalog.applied",
        payload: { mcpCatalogSessionId: catalogSessionId, revision: 0 },
      });
      const dispose = yield* decide(
        {
          type: "thread.mcp-catalog.dispose",
          commandId: CommandId.make("catalog-dispose"),
          threadId,
          mcpCatalogSessionId: catalogSessionId,
          revision: 0,
          disposedAt: "2026-01-01T00:00:03.000Z",
        },
        afterSessionSet,
      );
      expect(dispose).toMatchObject({
        type: "thread.mcp-catalog.disposed",
        payload: { mcpCatalogSessionId: catalogSessionId },
      });
      const afterDisposal = yield* projectEvent(afterSessionSet, {
        ...expectSingleEvent(dispose, "thread.mcp-catalog.disposed"),
        sequence: 2,
      });
      expect(afterDisposal.mcpCatalog?.sessions[0]?.disposedAt).toBe("2026-01-01T00:00:03.000Z");

      const lateReceipt = yield* Effect.flip(
        decide(
          {
            type: "thread.mcp-catalog.applied",
            commandId: CommandId.make("late-catalog-applied"),
            threadId,
            mcpCatalogSessionId: catalogSessionId,
            revision: 0,
            appliedCatalog: [],
            appliedAt: "2026-01-01T00:00:04.000Z",
          },
          afterDisposal,
        ),
      );
      expect(lateReceipt).toMatchObject({
        cause: expect.objectContaining({ _tag: "McpCatalogStaleSessionError" }),
      });
    }),
  );
});
