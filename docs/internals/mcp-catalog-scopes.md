...
This document records the scope and lifetime rules for the MCP catalog.
...
The gateway can add, update, and remove tools without replacing its MCP connection. That contract
does not imply that every provider refreshes a live catalog. External OpenCode catalogs remain
restart-required, and disconnect leaves a dynamic entry disabled because its API has no remove
operation.
