...
Submitted attachments remain in the thread history. Reverting a thread moves later answers and their attachments into an archived path that you can restore from **History**. You can attach up to eight files across one set of answers, using the same file size limits as the normal composer.
